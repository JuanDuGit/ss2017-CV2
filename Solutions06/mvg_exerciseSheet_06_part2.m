...
M2 = zeros(3*nPoints, nPoints + 1);
for i = 1:nPoints
   x2_hat = hat([x2(i) y2(i) 1]);
   x1_hat = hat([x1(i) y1(i) 1]);
   M(3*i-2 : 3*i, i) = x2_hat*R*[x1(i); y1(i); 1];
   M(3*i-2 : 3*i, nPoints+1) = x2_hat*T;
   M2(3*i-2 : 3*i, i) = x1_hat*R*[x2(i); y2(i); 1];
   M2(3*i-2 : 3*i, nPoints+1) = x1_hat*T;
end
...
gamma  = V(nPoints + 1, 1);
[V2,D2] = eig(M2' * M2);
lambda2 = V2(1:nPoints, 1);
gamma2  = V2(nPoints + 1, 1);
...
X2_tot = zeros(3,nPoints);
if (lambda1 >= zeros(nPoints,1))
if (lambda2 >= zeros(nPoints,1))
    display(R);
    display(T);
    display(lambda1);
    display(gamma);
